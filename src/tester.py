...
from utils import config_files, device_init, plot_images, load_file, path_names
    ...
    def display_images(self):
        try:
            ...
            if self.dataset not in ["train", "test"]:
                raise ValueError("Invalid dataset. Please choose between 'train' or 'valid'.")
            
            dataloader = dataset["train_dataloader"] if self.dataset == "train" else dataset["valid_dataloader"]
            
            best_model_path = os.path.join(path_names()["best_model_path"], "best_model.pth")
            model_state = torch.load(best_model_path, weights_only=False)["model_state_dict"]
